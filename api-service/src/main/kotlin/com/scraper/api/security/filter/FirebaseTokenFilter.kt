...
import com.scraper.api.security.auth.FirebaseAuthenticationToken
    ...
    override fun doFilterInternal(
        ...
        try {
            ...
            if (appUser == null) {
                ...
                logger.warn("Authenticated Firebase user not found in local DB: $firebaseUid")
            ...
            logger.debug("Successfully authenticated Firebase user: ${appUser.email}")

        } catch (e: ExecutionException) {
            // Token verification failed (expired, invalid signature, or revoked)
            logger.warn("Invalid Firebase ID Token: ${e.cause?.message ?: e.message}")
...
class FirebaseAuthenticationToken(
    val appUser: com.scraper.api.domain.user.AppUser,
...
fun com.scraper.api.domain.user.AppUser.getAuthorities(): Collection<org.springframework.security.core.GrantedAuthority> {
    // For the beta, we will grant a basic USER role to all authenticated users
    return listOf(org.springframework.security.core.authority.SimpleGrantedAuthority("ROLE_USER"))
}
