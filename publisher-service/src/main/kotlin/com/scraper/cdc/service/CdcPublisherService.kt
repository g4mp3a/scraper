...
import io.debezium.engine.DebeziumEngine.ChangeEvent
...
class CdcPublisherService(
    ...
    // Must match slot name configured in cdc_setup.sql
    ...
    override fun run(vararg args: String?) {
        try {
            ...
            logger.info("CDC Publisher initialized for topic: $topicName")

            // 2. Build Debezium Configuration
            val dbConfig = createDebeziumConfig()

            // 3. Create the Debezium Engine
            engine = DebeziumEngine.create<ChangeEvent<String, String>>()
                .using(dbConfig.asProperties())
                .notifying(this::handleChangeEvent) // Our custom handler
                .using(DebeziumEngine.Format.Json::class.java)
                .build()

            // 4. Submit the engine to run asynchronously
            executor.execute(engine)
            logger.info("Debezium Engine started, streaming changes from: $TARGET_TABLE")

        } catch (e: Exception) {
            logger.error("Failed to start Debezium CDC Publisher: ${e.message}", e)
            executor.shutdown()
    ...
    private fun handleChangeEvent(changeEvent: ChangeEvent<String, String>) {
        ...
        // We only care about INSERTs on our target table.
        if (changeEvent.destination.endsWith(TARGET_TABLE) && changeEvent.isCreate) {
            try {
                ...
                logger.debug("Published event ID: ${after.getAsJsonPrimitive("id").asLong}")

            } catch (e: Exception) {
                logger.error("Failed to parse or publish event to Pub/Sub. Stopping engine to prevent message loss: ${e.message}", e)
    ...
    @PreDestroy
    fun shutdown() {
        logger.info("Debezium Engine shutting down...")
        try {
            engine.stop()
            publisher.shutdown()
            executor.shutdownNow()
        } catch (e: Exception) {
            logger.error("Error during Debezium shutdown: ${e.message}", e)
        }
    }
}
